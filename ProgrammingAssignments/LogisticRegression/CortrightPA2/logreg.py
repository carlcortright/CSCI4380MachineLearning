...
class LogReg:
    ...
    def sg_update(self, train_example, iteration, use_tfidf=False):
        ...
        p = sigmoid(dot(self.beta, train_example.x)) # Set p
        print("Activation: %f" % p)
        print(self.step(iteration))
        delta = self.step(iteration) * ((train_example.y - p) * train_example.x)
        print("Delta = " + str(delta))
        # Set amount by which beta will change here.
        self.beta += delta

        if self.mu > 0:
            # Set regularization parameter and update
            ...
            for feat in train_example.nonzero:
                ...
                print("Raising %i to the %i" % (feat, gap))
                # Regularize beta parameter
    ...
    for pp in range(args.passes):
        for ii in train:
            ...
            if update_number % 5 == 1:
                ...
                trainingPlot.append(train_acc)
                updatePlot.append(update_number)

    # Print each word and its beta value
    betas = lr.get_betas()
    betas_dict = {}
    for i in range(0,len(betas)):
        betas_dict[betas[i]] = vocab[i]

    # Print the top hockey words
    print("Top hockey words:")
    for i in range(0, 10):
        top_ten = sorted(betas_dict.keys())
        print(str(betas_dict[top_ten[i]]) + " & " + str(top_ten[i]) + " \\\\")

    print()

    # Print the top baseball words
    print("Top baseball words: ")
    for i in range(0, 10):
        top_ten = sorted(betas_dict.keys(), reverse=True)
        print(str(betas_dict[top_ten[i]]) + " & " + str(top_ten[i]) + " \\\\")

    plt.ylim(0, 1.2)
    plt.scatter(updatePlot, trainingPlot)
    plt.title("Graph 2. Step = 0.1, Mu = 0.009, Passes = 6")
