...
class LogReg:
    ...
    def sg_update(self, train_example, iteration, use_tfidf=False):
        ...
        p = 0 #TODO Set p.
        print("Activiation: %f" % p)
        delta = 0 #TODO Set amount by which beta will change here.
        self.beta += delta

        if self.mu > 0:
            shrinkage = 0 #TODO Set regularization parameter here and update
            self.beta[0] *= shrinkage
            for feat in train_example.nonzero:
                #TODO regularize parameters in this loop
                gap = iteration - self.last_update.get(feat, -1)
                print("Raising %i to the %i" % (feat, gap))
                #TODO Regularize beta parameter here.
