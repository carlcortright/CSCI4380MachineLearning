...
rm iris.model
vw -f iris.model --passes=10 --cache_file=iris.cache --kill_cache  --oaa 3 --loss_function=hinge < vowpal.iris.training.data.vw

# Test the model
echo
echo
echo "Testing model..."
vw -i iris.model --cache_file test.cache -t -p ./predictions.txt < vowpal.iris.test.data.vw
